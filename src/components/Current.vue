<template>
  <div class="w-full px-6 md:px-20 md:py-0">
    ...
    <h2 class="font-display text-2xl text-left font-bold leading-tight md:leading-normal">Find out what's happening now in the world of Edgeryders</h2>
    ...
    <div class="w-full py-4 md:py-8 flex flex-col md:flex-row">
      ...
      <div class="w-full px-0 md:px-0 md:w-2/3 p-2 md:p-0 text-left events" v-if="active == 'events'">
        ...
        <div class="w-full px-0 md:px-0 md:w-2/3 p-2 md:p-0 text-left conversations" v-if="active == 'conversations'">
          <div class="entry" v-for="(entry, index) in conversations" v-if="index <= 5">
        ...
        <div class="w-full px-0 md:px-0 md:w-2/3 p-2 md:p-0 text-left conversations" v-if="active == 'calls'">
          <div class="entry" v-for="(entry, index) in calls" v-if="index <= 5">
            <div class="box" :style="{'background': colors[index]}">
              <div class="time">{{entry.created | formatDateShort}} @ {{entry.created | formatTime}}</div>
              <h3><a :href="'https://edgeryders.eu/t/' + entry.slug">{{entry.title}}</a></h3></div>
...
export default {
  ...
  props: ["events", "conversations", "calls"]
};
</script>
...
.conversations {
  columns: 2;
  @for $i from 1 to 10 {
  .entry:nth-child(#{$i}) { animation-delay: $i * 0.05s !important; }
}
  .entry {
  height: auto;
  opacity: 0;
